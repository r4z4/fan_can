defmodule FanCanWeb.CoreComponents do
  @moduledoc """
            ...
            >
              <div class="absolute top-6 right-5">
                <button
                  ...
                  type="button"
                  class="-m-3 flex-none p-3 opacity-80 hover:opacity-60"
  ...
  # Lime, Violet, Indigo, Cyan, Zinc, Fuchsia
  def flash(assigns) do
    ...
    <div
      ...
      class={[
        ...
        @kind == :thread && "bg-fuchsia-50 text-fuchsia-800 ring-fuchsia-500 fill-fuchsia-900",
        @kind == :new_post && "bg-fuchsia-50 text-fuchsia-800 ring-fuchsia-500 fill-fuchsia-900",
    ...
    >
      <p :if={@title} class="flex items-center gap-1.5 text-sm font-semibold leading-6">
        ...
        <.icon :if={@kind == :thread} name="book-open" class="h-4 w-4" />
        <.icon :if={@kind == :new_post} name="home" class="h-4 w-4" />
        <.icon :if={@kind == :election} name="academic-cap" class="h-4 w-4" />
  ...
  def flash_group(assigns) do
    ...
    <.flash kind={:thread} title="Thread" flash={@flash} />
    <.flash kind={:new_post} title="Thread Post" flash={@flash} />
  ...
  def simple_form(assigns) do
    ...
    <.form :let={f} for={@for} as={@as} {@rest}>
      <div class="mt-10 space-y-8 bg-slate-800">
        <%= render_slot(@inner_block, f) %>
        <div :for={action <- @actions} class="mt-2 flex text-white items-center justify-between gap-6">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(FanCanWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(FanCanWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  @doc """
  Custom Component for State Information Cards
  """
    ...
    <%= for item <- @items do %>
      <div class="card review-card">
        <div class="card-header">
          <%= for _ <- 0..item.ranking do %>
            <i class="bi bi-star-fill"></i>
          <% end %>
        </div>
        <div class="card-body">
          <h5 class="card-title">
            <strong>by: <%= item.title %></strong>
          </h5>
          <p class="card-text">
            <%= item.notes %>
          </p>
